import type { MemoryEdge, MemoryEdgeType, MemoryGraph } from "./types";
  ...
  suggestRelationships(): RelationshipSuggestion[] {
    ...
    for (const first of this.graph.nodes) {
      for (const second of this.graph.nodes) {
        ...
        const existing = this.graph.edges.some(
          (edge) =>
        ...
        if (first.type === "person" && second.type === "person") {
          suggestions.push({
            ...
            to: second.id,
            type: "related_to",
            confidence: this.calculateSimilarity(first.label, second.label),
          });
        }
      }
    }
